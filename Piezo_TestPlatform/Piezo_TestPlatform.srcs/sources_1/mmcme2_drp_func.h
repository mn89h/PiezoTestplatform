...
`endif
	 ...
	 mx    			=  2'b00; //CHANGED TO 2'b10 FOR ENABLING CLKOUT4_CASCADE!!!!!!!!
...
// This function takes in the divide, phase, and duty cycle
// setting to calculate the upper and lower counter registers.
function [37:0] mmcm_casc_count_calc
   (
      input [7:0] divide, // Max divide is 128
      input signed [31:0] phase,
      input [31:0] duty_cycle // Multiplied by 100,000
   ...
   begin
      count_calc = mmcm_count_calc(divide, phase, duty_cycle);
   `ifdef DEBUG
      $display("mx changed to 2'b10 (cascading)");
   `endif

      mmcm_casc_count_calc = { count_calc[37:26], 2'b10, count_calc[23:0] };
   end
endfunction
